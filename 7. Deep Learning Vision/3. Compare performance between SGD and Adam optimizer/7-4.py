...
# dictionary: [accuracy, loss, val_accuracy, val_loss]
...
plt.plot(hist_sgd.history['accuracy'], 'r--')   # red dot lines
plt.plot(hist_sgd.history['val_accuracy'], 'r') # red lines
plt.plot(hist_adam.history['accuracy'], 'b--')  # blue dot lines
plt.plot(hist_adam.history['val_accuracy'], 'b')# blue lines
plt.title('Comparison of SGD and Adam optimizer')
plt.ylim(0.7, 1.0)  # limitation of y axis
plt.xlabel('epoch')
plt.ylabel('accuracy')
plt.legend(['train_sgd', 'val_sgd', 'train_adam', 'val_adam'])  # makes legend(explains the meaning of colors, symbols, or patterns)
plt.grid()
plt.show()
